...
namespace alhadaProject.Controllers
{
    public class AdminController : Controller
    {
        ...
        public bool isLegal()
        {
            ...
            if (HttpContext.Session.GetString("userType") != "Admin") return false;
        ...
        public IActionResult MyClient()
        {
            ...
            var r =  (from Client in _context.Clients
            ...
            {
                list.Add(new ClientViewModel { Id = r[i].Id , Ref = r[i].Ref , AddedDate = r[i].AddedDate, Archeive = r[i].Archeive , Buy = r[i].Archeive
                , ClientName = r[i].ClientName, Mobile = r[i].Mobile ,
                    EmpName = r[i].EmpName ,
            ...
            {
                    selectList.Add(new SelectListItem { Text = item.EmpName, Value = Convert.ToString( item.Id)  } );
            }
        ...
        {
            var recs = _context.Employees.ToList();
            return View(recs);
        }
        public IActionResult AddUsers()
        {
            ViewBag.items = new List<string> { "Admin" , "Emp" };
            return View();
        }

        [HttpPost]
        public IActionResult AddUsers(Employee employee)
        {

            var rec = _context.Employees.Where(tbl=> tbl.Email == employee.Email).AsNoTracking().FirstOrDefault();
            if (rec != null)
            {
                ViewBag.error = "البريد الالكتروني موجود مسبقا";
                ViewBag.items = new List<string> { "Admin", "Emp" };
        ...
        [HttpPost]
        public IActionResult EditUser(int id , Employee employee)
        {

            var rec = _context.Employees.Where(tbl=>tbl.Id== id).AsNoTracking().FirstOrDefault();
            employee.Password = rec.Password;
        ...
        {
            var recs =_context.Inboxes.ToList();
            ViewBag.msg = recs;

            return View();
        }

        public IActionResult readInbox(int id)
        {
            var recs = _context.Inboxes.Where(tbl=>tbl.Id==id).ToList().FirstOrDefault();
            ViewBag.msg = recs;
            return View();
        }
